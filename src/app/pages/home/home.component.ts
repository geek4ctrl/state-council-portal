...
interface HeroSlide { id:number; titleKey:string; subtitleKey:string; descriptionKey:string; buttonKey:string; image:string; }
interface PresidentSlide { id:number; titleKey:string; image:string; paragraphKeys:string[]; }
...
@Component({
  ...
  template: `
    ...
    <div class="page-wrap">
      ...
      <!-- ═══ HERO ═══ -->
      <section class="hero">
        <div class="hero-bg" [style.background-image]="'url('+heroSlides[currentSlide()].image+')'"></div>
        <div class="hero-fog"></div>
        <canvas class="hero-canvas" #heroCanvas></canvas>
        <div class="hero-body">
          <div class="container">
            <div class="hero-text">
              <div class="hero-tag" [attr.data-slide]="currentSlide()">{{ heroSlides[currentSlide()].subtitleKey | i18n }}</div>
              <h1 class="hero-h1" [attr.data-slide]="currentSlide()">{{ heroSlides[currentSlide()].titleKey | i18n }}</h1>
              <div class="hero-btns">
                <button class="hbtn primary mag-btn" (mousemove)="mag($event)" (mouseleave)="magOut($event)" (click)="ripple($event)">
                  <span>{{ heroSlides[currentSlide()].buttonKey | i18n }}</span>
                  <div class="hbtn-side"></div>
                </button>
                <button class="hbtn ghost mag-btn" (mousemove)="mag($event)" (mouseleave)="magOut($event)" (click)="ripple($event)">
        ...
        <div class="hero-nav">
          <div class="container">
            <div class="hero-controls">
              <button class="h-arr" (click)="prevSlide()" (mouseenter)="arrHover($event,true)" (mouseleave)="arrHover($event,false)">
                <div class="arr-face arr-front"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg></div>
              ...
              <button class="h-arr" (click)="nextSlide()" (mouseenter)="arrHover($event,true)" (mouseleave)="arrHover($event,false)">
                <div class="arr-face arr-front"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></div>
                <div class="arr-face arr-top"></div>
                <div class="arr-face arr-bottom"></div>
              </button>
            </div>
      ...
      <section class="kf-section">
        ...
        <div class="container">
          ...
          <div class="kf-grid">
            <div class="kf-card tilt-card" style="--i:0" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              ...
              <div #caseVolumeChart class="kf-chart" role="img" [attr.aria-label]="'home.keyFacts.cards.volume.aria' | i18n"></div>
              <div class="card-edge-r"></div><div class="card-edge-b"></div>
            </div>
            <div class="kf-card tilt-card" style="--i:1" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="tilt-shadow"></div>
              <div class="kf-meta">
              ...
              <div #processingTimeChart class="kf-chart" role="img" [attr.aria-label]="'home.keyFacts.cards.processing.aria' | i18n"></div>
              <div class="card-edge-r"></div><div class="card-edge-b"></div>
            </div>
            <div class="kf-card tilt-card" style="--i:2" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="tilt-shadow"></div>
              <div class="kf-meta">
              ...
              <div #decisionsTypeChart class="kf-chart" role="img" [attr.aria-label]="'home.keyFacts.cards.decisions.aria' | i18n"></div>
              <div class="card-edge-r"></div><div class="card-edge-b"></div>
            </div>
          </div>
        </div>
      ...
      <section class="exp-section">
        ...
        <div class="container">
          ...
          <div class="exp-grid">
            <a class="exp-card tilt-card accent-civil" href="#" style="--i:0" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="exp-card-accent-bar"></div>
              <div class="exp-card-depth"></div>
              <div class="exp-icon-wrap">
                <div class="exp-icon float-icon">
                  <svg viewBox="0 0 64 64" fill="currentColor"><path d="M38 8L26 8C24 8 22 10 22 12L22 52C22 54 24 56 26 56L38 56C40 56 42 54 42 52L42 12C42 10 40 8 38 8Z"/><path d="M32 12L28 16L36 16L32 12Z" fill="white"/></svg>
              ...
              <span class="exp-cta">{{ 'home.expertise.cta' | i18n }} <span class="cta-arr">→</span></span>
            </a>
            <a class="exp-card tilt-card accent-family" href="#" style="--i:1" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="exp-card-accent-bar"></div>
              <div class="exp-card-depth"></div>
              <div class="exp-icon-wrap">
                <div class="exp-icon float-icon">
                  <svg viewBox="0 0 64 64" fill="currentColor"><path d="M24 40C16 40 10 44 10 50V56H38V50C38 44 32 40 24 40Z"/><path d="M40 40C38 40 36 40.4 34 41C37 43 38 46 38 50V56H54V50C54 44 48 40 40 40Z"/><circle cx="24" cy="26" r="10"/><circle cx="42" cy="24" r="8"/></svg>
                  <div class="exp-icon-pulse"></div>
                </div>
            ...
            <a class="exp-card tilt-card accent-public" href="#" style="--i:2" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="exp-card-accent-bar"></div>
              <div class="exp-card-depth"></div>
              <div class="exp-icon-wrap">
                <div class="exp-icon float-icon">
                  <svg viewBox="0 0 64 64" fill="currentColor"><path d="M32 8L18 14V28C18 40 26 48 32 52C38 48 46 40 46 28V14L32 8Z M32 12L42 16V28C42 38 36 44 32 48C28 44 22 38 22 28V16L32 12Z"/><rect x="28" y="24" width="8" height="16" fill="white"/><rect x="24" y="32" width="16" height="4" fill="white"/></svg>
                  <div class="exp-icon-pulse"></div>
                </div>
            ...
            <a class="exp-card tilt-card accent-labor" href="#" style="--i:3" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="exp-card-accent-bar"></div>
              <div class="exp-card-depth"></div>
              <div class="exp-icon-wrap">
                <div class="exp-icon float-icon">
                  <svg viewBox="0 0 64 64" fill="currentColor"><rect x="22" y="28" width="20" height="20" rx="2"/><rect x="26" y="20" width="12" height="10" rx="2"/><circle cx="32" cy="38" r="3" fill="white"/></svg>
                  <div class="exp-icon-pulse"></div>
                </div>
            ...
            <a class="exp-card tilt-card accent-criminal" href="#" style="--i:4" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="exp-card-accent-bar"></div>
              <div class="exp-card-depth"></div>
              <div class="exp-icon-wrap">
                <div class="exp-icon float-icon">
                  <svg viewBox="0 0 64 64" fill="currentColor"><path d="M32 8C24 8 18 14 18 22C18 30 24 36 32 36C40 36 46 30 46 22C46 14 40 8 32 8Z"/><path d="M46 18C46 14 43 11 39 11L25 11C21 11 18 14 18 18L18 46C18 50 21 53 25 53L39 53C43 53 46 50 46 46L46 18Z"/><path d="M28 18L32 24L36 18" stroke="#7a4d66" stroke-width="2" fill="none"/></svg>
                  <div class="exp-icon-pulse"></div>
                </div>
            ...
            <a class="exp-card tilt-card accent-property" href="#" style="--i:5" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              <div class="tilt-shine"></div>
              <div class="exp-card-accent-bar"></div>
              <div class="exp-card-depth"></div>
              <div class="exp-icon-wrap">
                <div class="exp-icon float-icon">
                  <svg viewBox="0 0 64 64" fill="currentColor"><rect x="16" y="32" width="32" height="20"/><polygon points="32,12 16,32 48,32"/><rect x="28" y="24" width="8" height="8" fill="white"/></svg>
                  <div class="exp-icon-pulse"></div>
                </div>
      ...
      <section class="ct-section">
        <div class="container">
          <div class="ct-bar">
            <div class="ct-item" style="--i:0" (mouseenter)="ctPop($event)" (mouseleave)="ctUnpop($event)">
              <div class="ct-icon-box">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="28" height="28"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            ...
            <div class="ct-item" style="--i:1" (mouseenter)="ctPop($event)" (mouseleave)="ctUnpop($event)">
              <div class="ct-icon-box">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="28" height="28"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                <div class="ct-icon-ring"></div>
              </div>
              <div class="ct-text">
            ...
            <div class="ct-sep"></div>
            <div class="ct-item" style="--i:2" (mouseenter)="ctPop($event)" (mouseleave)="ctUnpop($event)">
              <div class="ct-icon-box">
          ...
          <div class="pres-layout">
            <div class="pres-img-stage" (mousemove)="tilt($event)" (mouseleave)="tiltReset($event)">
              ...
              <div class="pres-img-frame">
                <img [src]="presSlides[currentPresSlide()].image" [alt]="'home.president.imageAlt' | i18n">
            ...
            <div class="pres-content">
              ...
              <button class="pres-btn mag-btn" (mousemove)="mag($event)" (mouseleave)="magOut($event)" (click)="ripple($event)">
                <span>{{ 'home.president.cta' | i18n }}</span>
                <div class="pres-btn-aura"></div>
              </button>
              <div class="pres-dots">
                @for(s of presSlides; track s.id; let i=$index){
                  <button class="pres-dot" [class.on]="i===currentPresSlide()" (click)="gotoPresSlide(i)">
  ...
  styles: [`
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    /* ━━━━━━━━━━━━━━ KEYFRAMES ━━━━━━━━━━━━━━ */
    @keyframes sphereSpin{to{transform:rotateX(-20deg) rotateY(360deg)}}
    ...
    @keyframes heroTagIn{from{opacity:0;transform:translateY(20px) rotateX(40deg)}to{opacity:1;transform:translateY(0) rotateX(0)}}
    @keyframes heroH1In{from{opacity:0;transform:translateY(30px) rotateX(30deg) scale(.96)}to{opacity:1;transform:translateY(0) rotateX(0) scale(1)}}
    @keyframes heroBtnsIn{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
    ...
    @keyframes particleDrift{0%{opacity:0;transform:translateY(0) scale(0)}20%{opacity:.8}100%{opacity:0;transform:translateY(-160px) scale(1.5)}}
    ...
    @keyframes orbitDot{from{transform:rotate(0deg) translateX(26px) rotate(0deg)}to{transform:rotate(360deg) translateX(26px) rotate(-360deg)}}
    @keyframes beamRise{0%{height:0;opacity:0}100%{height:60%;opacity:.4}}
    ...
    @keyframes iconIconBob{0%,100%{transform:translateY(0) rotateY(0)}50%{transform:translateY(-6px) rotateY(12deg)}}
    @keyframes auroraShift{0%,100%{transform:translate(0,0) scale(1)}50%{transform:translate(40px,-30px) scale(1.1)}}
    @keyframes presImgIn{from{opacity:0;transform:translateX(-60px) rotateY(20deg)}to{opacity:1;transform:translateX(0) rotateY(0)}}
    @keyframes presContentIn{from{opacity:0;transform:translateX(60px)}to{opacity:1;transform:translateX(0)}}
    @keyframes dotWave{from{transform:scale(1);opacity:.8}to{transform:scale(3);opacity:0}}
    ...
    .hero-h1{
      font-size:2.9rem;font-weight:900;line-height:1.12;margin-bottom:28px;
    ...
    .hbtn span{position:relative;z-index:2;}
    .hbtn-side{
      position:absolute;bottom:0;left:0;right:0;height:5px;
      transform:translateY(100%) translateZ(-5px);
      transition:opacity .3s ease;
    }
    .hbtn.primary{background:#C82333;color:#fff;border:none;box-shadow:0 6px 0 #8B1020,0 12px 30px rgba(200,35,51,.4);}
    .hbtn.primary .hbtn-side{background:#8B1020;}
    .hbtn.primary:hover{transform:translateY(-4px) rotateX(8deg);box-shadow:0 10px 0 #8B1020,0 20px 40px rgba(200,35,51,.5);}
    .hbtn.primary:active{transform:translateY(2px) rotateX(-4deg);box-shadow:0 3px 0 #8B1020,0 6px 15px rgba(200,35,51,.3);}
    .hbtn.ghost{background:transparent;color:#fff;border:1.5px solid rgba(255,255,255,.5);box-shadow:0 6px 0 rgba(255,255,255,.1),0 12px 30px rgba(0,0,0,.2);}
    .hbtn.ghost:hover{transform:translateY(-4px) rotateX(8deg);background:rgba(255,255,255,.1);border-color:#fff;}
    .hbtn.ghost:active{transform:translateY(2px) rotateX(-4deg);}
    .hbtn::before{content:'';position:absolute;inset:0;background:linear-gradient(105deg,transparent 40%,rgba(255,255,255,.2) 50%,transparent 60%);transform:translateX(-120%) skewX(-20deg);pointer-events:none;}
    .hbtn:hover::before{animation:shimmerSweep .6s ease forwards;}
    .mag-btn{transition:transform .25s cubic-bezier(.23,1,.32,1);}

    .hero-nav{position:absolute;bottom:32px;left:0;right:0;z-index:3;}
    .hero-controls{display:flex;align-items:center;gap:30px;justify-content:flex-start;}
    .h-arr{
    ...
    .tilt-card{
      ...
      opacity:0;animation:cardIn .7s cubic-bezier(.23,1,.32,1) calc(var(--i,0)*.1s) forwards;
      position:relative;overflow:hidden;
    }
    .tilt-shine{
      position:absolute;inset:0;border-radius:inherit;pointer-events:none;z-index:10;
      background:linear-gradient(105deg,transparent 0%,rgba(255,255,255,.0) 45%,rgba(255,255,255,.18) 50%,rgba(255,255,255,.0) 55%,transparent 100%);
      transform:translateX(-120%) skewX(-20deg);
    }
    .tilt-shadow{position:absolute;inset:0;border-radius:inherit;pointer-events:none;z-index:-1;box-shadow:0 30px 60px rgba(0,0,0,.0);transition:box-shadow .5s ease;}
    ...
    .kf-card{
      ...
      box-shadow:0 12px 40px rgba(26,41,66,.12),inset 0 1px 0 rgba(255,255,255,.7);
    }
    ...
    .ql-card.flipped .flip-inner{transform:rotateY(180deg);}
    ...
    .ql-info h3{font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.7px;color:#1a1a1a;margin-bottom:6px;}
    .ql-info p{font-size:.75rem;color:#4b5563;line-height:1.5;flex:1;}
    .ql-cta{font-size:.72rem;font-weight:700;color:#1a2942;display:inline-flex;align-items:center;gap:4px;margin-top:6px;transition:gap .3s ease;}
    .ql-cta:hover{gap:8px;}
    ...
    .exp-card-accent-bar{
      position:absolute;left:0;top:3px;bottom:0;width:3px;
      background:linear-gradient(to bottom,var(--accent),transparent);
      transform:scaleY(0);transform-origin:top;transition:transform .45s cubic-bezier(.23,1,.32,1);
      border-radius:0 0 0 14px;
    }
    .exp-card:hover .exp-card-accent-bar{transform:scaleY(1);}
    .exp-card-depth{
      position:absolute;bottom:-4px;left:4px;right:-4px;top:4px;
      background:linear-gradient(135deg,var(--accent),transparent);
      border-radius:14px;z-index:-1;opacity:0;
      transition:opacity .4s ease,transform .4s ease;
    }
    .exp-card:hover .exp-card-depth{opacity:.12;transform:translate(2px,2px);}
    ...
    .float-icon:nth-child(odd){animation-delay:.5s;}
    ...
    .exp-desc{font-size:.86rem;color:#6a6a6a;line-height:1.6;margin-bottom:12px;}
    .exp-list{list-style:none;padding:0;flex:1;}
    .exp-list li{display:flex;align-items:baseline;gap:8px;padding:5px 0;color:#5b5b5b;font-size:.87rem;line-height:1.5;}
    ...
    .ct-bar{display:flex;align-items:stretch;}
    .ct-item{
    ...
    .pres-img-stage{
      ...
      animation:presImgIn .9s cubic-bezier(.23,1,.32,1) .2s both;
      transform-style:preserve-3d;
    }
    .pres-img-frame{position:absolute;inset:0;}
    .pres-img-frame img{width:100%;height:100%;object-fit:cover;transition:transform .6s ease;}
    .tilt-card:hover .pres-img-frame img{transform:scale(1.05);}
    .pres-img-overlay{
      position:absolute;inset:0;
      background:linear-gradient(180deg,transparent 40%,rgba(0,18,45,.5) 100%);
      pointer-events:none;
    }
    .pres-img-grid{
      position:absolute;inset:0;
      background-image:linear-gradient(rgba(255,255,255,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.04) 1px,transparent 1px);
      background-size:30px 30px;opacity:0;transition:opacity .4s ease;pointer-events:none;
    }
    .tilt-card:hover .pres-img-grid{opacity:1;}
    .pres-img-shadow{
    ...
    .pres-content{animation:presContentIn .9s cubic-bezier(.23,1,.32,1) .4s both;}
    .pres-title{
      font-size:2rem;font-weight:900;color:#2C3E50;margin-bottom:24px;
    ...
    .pres-dot{
      ...
      background:#d0d0d0;border:none;border-radius:50%;cursor:pointer;
    ...
    /* ━━━━━━━━━━━━━━ NEWSLETTER ━━━━━━━━━━━━━━ */
    .nl-section{padding:80px 0;background:#fff;position:relative;overflow:hidden;}
    .nl-scanlines{
      position:absolute;inset:0;pointer-events:none;
      background:repeating-linear-gradient(0deg,rgba(191,152,116,.03) 0,rgba(191,152,116,.03) 1px,transparent 1px,transparent 40px);
    ...
    .nl-img-scan{
      ...
      background:linear-gradient(90deg,transparent,rgba(191,152,116,.6),transparent);
    ...
    .nl-link:hover .nl-underline{width:100%;}
    .nl-card-glow{
      position:absolute;inset:0;border-radius:inherit;
      background:radial-gradient(circle at 50% 100%,rgba(191,152,116,.1),transparent 70%);
    ...
    @media(max-width:1023px){
      ...
      .offer-section,.exp-section,.pres-section,.nl-section{padding:56px 0;}
      .sec-title{font-size:1.8rem;}
      .kf-grid{grid-template-columns:repeat(2,1fr);}
      .ct-bar{flex-direction:column;} .ct-sep{width:100%;height:1px;background:linear-gradient(to right,transparent,rgba(191,152,116,.5),transparent);}
      .ct-item{padding:22px 20px;}
      .pres-layout{grid-template-columns:1fr;gap:40px;}
      .pres-img-stage{max-width:400px;margin:0 auto;height:400px;}
    }
    @media(max-width:767px){
      ...
      .h-arr{width:40px;height:40px;} .h-arr svg{width:22px;height:22px;}
      .h-dot{width:38px;}
      .kf-grid{grid-template-columns:1fr;}
      .ql-row{grid-template-columns:1fr;gap:12px;}
      .offer-grid{grid-template-columns:1fr;gap:20px;}
      .offer-section,.exp-section,.pres-section,.nl-section{padding:46px 0;}
...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  private readonly onVisibility = () => { if (!document.hidden) this.charts.forEach(c => c.reflow()); };
  ...
  ql = [
    ...
    { key:'excerpts', img:'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=120&h=120&fit=crop', title:'home.quickLinks.items.excerpts.title', body:'home.quickLinks.items.excerpts.body', action:'home.quickLinks.items.excerpts.action' },
    { key:'report', img:'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=120&h=120&fit=crop', title:'home.quickLinks.items.report.title', body:'home.quickLinks.items.report.body', action:'home.quickLinks.items.report.action' },
  ...
  private initHeroCanvas() {
    ...
    for (let i = 0; i < 55; i++) {
      particles.push({ x:Math.random()*W, y:Math.random()*H, vx:(Math.random()-.5)*.4, vy:-Math.random()*.5-.1, size:Math.random()*2.5+.5, alpha:Math.random()*.6+.2, decay:Math.random()*.003+.001 });
    }
    const draw = () => {
  ...
  arrHover(e: MouseEvent, enter: boolean) {
    const el = e.currentTarget as HTMLElement;
    if (enter) { el.style.transform = 'translateZ(8px) scale(1.12)'; }
  ...
  nextSlide() { this.currentSlide.update(c => c === this.heroSlides.length-1 ? 0 : c+1); }
  prevSlide() { this.currentSlide.update(c => c === 0 ? this.heroSlides.length-1 : c-1); }
  gotoSlide(i: number) { this.currentSlide.set(i); }

  nextPresSlide() { this.currentPresSlide.update(c => c === this.presSlides.length-1 ? 0 : c+1); }
  gotoPresSlide(i: number) { this.currentPresSlide.set(i); }

  private setupObservers() {
    if (typeof ResizeObserver !== 'undefined') {
      this.resizeObserver = new ResizeObserver(() => this.charts.forEach(c => c.reflow()));
      [this.chartA, this.chartB, this.chartC].forEach(r => this.resizeObserver?.observe(r.nativeElement));
    }
  ...
  private renderCharts() {
    ...
    this.charts = [
      Highcharts.chart(this.chartA.nativeElement, {
        chart:{type:'column',backgroundColor:'transparent',height:220,spacing:[10,10,0,10]},
      ...
      Highcharts.chart(this.chartC.nativeElement, {
        ...
        legend:{align:'center',verticalAlign:'bottom',itemStyle:{color:'#1f2937',fontWeight:'600',fontSize:'12px'}},
